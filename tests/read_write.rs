...
fn it_initializes_nfc_manager() {
    ...
    unsafe {
        assert_eq!(
            ...
            0,
            "Failed to initialize nfc manager.\
            Note: this test requires a card reader to be attached"
        );
...
fn it_reads_and_writes_ndef_text() -> Result<(), Box<dyn Error>> {
    ...
    let mut tag_callback = nfcTagCallback_t {
        onTagArrival: unsafe {
    ...
    unsafe {
        ...
        assert_eq!(
            ...
            1,
            "Failed to activate NFC manager. Is a NFC reader attached?"
        );
    }
    ...
    let tag_info = match rx.recv_timeout(std::time::Duration::from_millis(TAG_READ_TIMEOUT_MS)) {
        ...
        Err(_) => {
            return do_cleanup(Err(
                "Timedout waiting for tag. Is there a tag near the reader?".into(),
            ))
        }
    };
    ...
    unsafe {
        let res = nfcTag_writeNdef(
            tag_info.handle,
            ndef_content.as_mut_ptr(),
            ndef_content_len.try_into().unwrap(),
        );
        if res != 0 {
            return do_cleanup(Err(format!(
                "Failed to write to tag. Got {}",
                ndef_content_len
            )
            .into()));
        }
